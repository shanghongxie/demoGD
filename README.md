...
## Details
Consider a linear regression model with $p$ predictors: $Y=X\Theta+\epsilon$, where $X$ is an $n\times p$ data matrix, $\Theta=(\theta_1, \dots, \theta_p)^T$ represents the parameters, and error term $\epsilon=(\epsilon_1, \dots, \epsilon_n)^T$, where $\epsilon_i$'s are independent and $\epsilon_i\sim N(0, \sigma^2)$.
